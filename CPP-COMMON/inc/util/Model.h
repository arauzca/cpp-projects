...
private:
    ...
    vector<Texture> loadMaterialTextures( aiMaterial *mat, aiTextureType type, const string & typeName )
    {
        ...
        for ( GLuint i = 0; i < mat->GetTextureCount( type ); i++ )
        {
            ...
            auto skip = static_cast<GLboolean>( false );
            
            for (auto &j : textures_loaded) {
                if(j.path == str )
                {
                    textures.push_back(j);
                    skip = static_cast<GLboolean>( true ); // A texture with the same filepath has already been loaded, continue to next one. (optimization)
